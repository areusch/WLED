...
<style>body{margin:0}#canv{background:black;filter:brightness(175%);width:100%;height:100%;position:absolute;display:flex;justify-content:center;align-items:center}#doorFilter{color:#f00;font-size:16pt}</style>
</head>
<body>
<div id=canv>
<div id=doorFilter></div>
</div>
<script>update();var tmout=null;function update()
{if(document.hidden){clearTimeout(tmout);tmout=setTimeout(update,250);return;}
fetch('/json/live').then(res=>{if(!res.ok){clearTimeout(tmout);tmout=setTimeout(update,2500);}
...
str+=")";document.getElementById("canv").style.background=str;document.getElementById("doorFilter").innerText=json['door'];clearTimeout(tmout);tmout=setTimeout(update,40);}).catch(function(error){clearTimeout(tmout);tmout=setTimeout(update,2500);})}</script>
</body>
</html>)=====";
